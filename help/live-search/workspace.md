...
ヘッドレスストアフロントに追加する必要がある必要なイベントについて詳しくは、次のドキュメントを参照してください。
...
>[!TIP]
>
>検索可能な属性を選択することは、検索品質に大きな影響を与えます。 検索可能な属性の選択と一般的な設定の問題の回避に関する詳細なガイダンスについては、_ベストプラクティスガイド_&#x200B;の「[製品メタデータの活用](best-practice.md#leverage-product-metadata)」を参照してください。
...
この動作は、色やサイズなどの属性には適していますが、`new_arrival`、`product_ranking`、`promotion_label`、カスタム価格など、製品全体を説明する属性に対して予期しない結果が生じる可能性があります。

例えば、設定可能な製品（SKU-001）に`new_arrival = true`があり、子バリアント SKU-001-01に`new_arrival = false`があるとします。 親製品のバリアント値が集計されると、SKU-001には`new_arrival = true`と`new_arrival = false`の両方のインデックスが作成されます。 その結果、各値が異なるバリアントに適用されても、親の製品は、いずれかの値の検索結果に表示されます。
...
- 結果をさらに絞り込むには、レイヤー検索の2番目のレイヤーで`startsWith`と`contains`検索インデックスを使用します。
...
- **には検索インデックスが含まれています** - 「含まれている」インデックスを使用して属性を検索します。 この新しい機能により、次のことが可能になります。
  ...
  - 大きな文字列内のクエリを検索しています。 例えば、買い物客が「HAPE-123」という文字列で「PE-123」という商品番号を検索した場合です。
    ...
    >[!NOTE]
    >
    >この検索タイプは、単語の先頭を一致させることでオートコンプリートをサポートする既存の[語句検索](https://developer.adobe.com/commerce/webapi/graphql/schema/live-search/queries/product-search#phrase)とは異なります。 たとえば、product属性値が「outdoor pants」の場合、「out」と「pan」は値の単語の始まりと一致するため、「out pan」の検索結果がフレーズ検索で返されます。 単語の中に文字列が存在するため、「またはアリ」の結果は返されません。 Aには、単語内の任意の場所で検索テキストが一致するので、「またはアリ」の結果が返されます。
...
>[!NOTE]
>
>製品属性は、必要なプロパティを持つ場合のみフィルタリング可能です。*検索で使用= Yes*、*検索結果で使用する階層化されたナビゲーション=yes*、*階層化されたナビゲーションで使用= フィルター可能（結果を含む）*。 これらのプロパティが見つからないか、正しく設定されていない場合、属性はファセット設定に表示されません。 設定手順については、[&#x200B; ファセットの追加](facets-add.md#step-1-add-a-facet)を参照してください。
...
>[!IMPORTANT]
>
>Adobeでは、Live Search 4.0.0でデフォルトで有効になっている商品リストウィジェットを使用することをお勧めします。 ウィジェットは、今後のリリースでアダプタ実装を置き換えることを目的としています。 詳しくは、[製品リストウィジェットの有効化](install.md#enable-product-listing-widgets)を参照してください。
...
次の表は、Luma サンプルデータに固有のものを含め、システム以外の属性のデフォルトの検索およびフィルター可能なプロパティを示しています。 [!DNL Live Search]とネイティブ Adobe Commerceの両方で属性を検索できるようにするには、*検索で使用*&#x200B;属性プロパティを`Yes`に設定します。
