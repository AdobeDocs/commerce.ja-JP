...
>[!NOTE]
>
>製品属性は、製品属性の設定に必要なプロパティ *Use in Search = No*、*Use in Search Results Layered Navigation=yes*、および *Use in Layered Navigation=Filterable （with results）* がある場合にのみフィルタリングできます。 これらのプロパティがない場合や正しく設定されていない場合、ファセット設定に属性が表示されません。 設定手順については、[ ファセットの追加 ](facets-add.md#add-a-facet) を参照してください。
