...
- 検索アダプターは、[&#x200B; 4.0.0 以降 &#x200B;](release-notes.md#live-search-400) 非推奨 [!DNL Live Search] になりました。製品リストページ（PLP）ウィジェットは、今後のすべての実装でサポートされるソリュ [!DNL Live Search] ションです。 検索アダプタは、セキュリティ関連の更新のみを受け取ります。 PLP ウィジェットへの移行については、[&#x200B; 移行ガイド &#x200B;](migrate-to-plp.md) を参照してください。
