...
- 適切に設計された検索エクスペリエンスは、顧客との信頼を構築し、必要なものを見つけることができるという自信を植え付けます。
...
- データ収集
   ...
   - [Adobe Commerce イベント コレクター ](https://developer.adobe.com/commerce/services/shared-services/storefront-events/reference/event-framework/)
...
検索可能にする属性を選択すると、検索品質に大きな影響を与えます。 検索可能な属性が多すぎると、返される結果の数が増えた場合でも、関連性が低下し、予期しない一致が発生する可能性があります。 この節では、カバレッジと関連性のバランスを取るために検索可能な属性を意図的に選択する方法について説明します。

**推奨される検索可能な属性：**

- **製品名** – 概要。製品を直接説明します。
- **プライマリの説明** – 買い物客が一致すると期待される簡潔な製品詳細。
- **ブランド** – 買い物客は頻繁にブランドで検索します。
- **モデル番号/スタイル** – 明確な目的を持つ特定の識別子。
- **主な特長** – 重要な際立った特性（「防水」、「ワイヤレス」など）
- **素材/ファブリック** - ファッションおよび家具カテゴリ用。

**これらの属性を検索可能にしないでください。**

- **長い説明または仕様** - テキストが多すぎると、ノイズが発生し、予期しない一致が発生します。
- **カテゴリパス** – 幅広い分類用語により、無関係な結果が生じる可能性があります。
- **混合文字を含む内部 SKU コード** – 部分一致で誤検知を作成します。
- **管理フィールド** – 買い物客に関係のない内部メモまたはウェアハウスコード。
- **HTMLのコンテンツまたはタグの書式設定** - テクニカルコンテンツは関連度を向上させません。

#### 検索可能な属性が正しくないことに起因する一般的な問題

間違った属性を検索できるようにすると、買い物客の不満が生じ、サポートエスカレーションが発生する場合があります。

| 例 | シナリオ | 推奨事項 |
|---------|----------|----------------|
| **ステム化とオートコンプリートの副作用** | 販売者は、長い製品の説明を検索可能にします。 買い物客が「can」（コンテナを探す）を検索します。 ステミングと部分的なマッチングにより、「アメリカ」、「キャノピー」、「キャンバス」など、説明の中で大きな単語の一部として「can」を含む製品が表示されます。 買い物客のインテントサーフェスに関連のない製品が検索の信頼性を失う。 | 検索可能なフィールドを、製品名やメインの説明などの目的の多い属性に減らします。 上位の検索用語を検証して、問題のある一致を特定します。 検索マーチャンダイジングルールまたはリダイレクトを使用して、特定の既知のエッジケースを処理します。 |
| **人気度ランキングでマッチングのノイズを増幅** | マーチャントは、ランキングを「最も購入済み」に設定し、カテゴリパスと長い説明を検索可能な属性として含めます。 買い物客が「ノートパソコンのバッグ」を検索する。 幅広いマッチングは、ラップトップのバッグ、ラップトップのアクセサリー、他の目的のためのバッグ、およびラップトップ自体を返します。 ノートパソコンはノートパソコンのバッグよりも購入頻度が高いため、最上位にランク付けされます。 システムが設定どおりに動作している場合でも、買い物客はランキングが正しくないと認識します。 | カテゴリ パスなどのノイズの多い検索可能な属性を削除します。 一致セットがより正確になったら、人気度ベースのランキング戦略を適用します。 検索分析を監視して、このパターンが発生するクエリを特定します。 |
| **カテゴリパスで偽陽性が作成される** | マーチャントは、完全なカテゴリパスを検索可能にします（例：「ホーム/キッチン/アプライアンス/小型アプライアンス」）。 買い物客が「ホームオフィスデスク」を検索する。 「ホーム」カテゴリの製品は、カテゴリパスに「ホーム」が存在するので、キッチンアイテムであっても一致します。 キッチン器具、家庭用装飾、およびその他の無関係の製品は、結果として現れ、希釈関連性がある。 | カテゴリパスを検索可能にしないでください。代わりにファセットを使用して、買い物客がカテゴリでフィルタリングできるようにします。 検索戦略に不可欠なカテゴリフィルタリングの場合は、検索可能な属性ではなくマーチャンダイジングルールを使用して実装します。 |
...
- **最上位重量（9-10）:** 製品名、ブランド
- **Mediumの重み（6 ～ 8）:** モデル番号、主な説明、主な機能
- **低重量（3-5）:**&#x200B;セカンダリ説明、資料、仕様

各製品の検索可能な各属性内に関連するコンテンツが含まれていることを確認します。 大量のコンテンツがある属性は、検索結果の関連性を低下させる可能性があるので、検索可能として設定しないことをお勧めします。

#### トラブルシューティング

検索結果がランダムまたは無関係と感じる場合は、製品の不具合としてエスカレーションする前に、このチェックリストを使用します。

1. **検索可能な属性の設定を確認：**

   - 現在検索可能として設定されているすべての属性をリストします。
   - 広い属性やノイズの多い属性（長い説明、カテゴリパス、管理フィールド）を特定します。
   - 買い物客の意図を表さない属性から検索可能ステータスを削除します。

1. **クエリの一致をコア属性と照合して検証する：**

   - 最も一般的な検索クエリをテストします。
   - 結果が接線方向のテキストではなく、主に製品名とブランドに一致することを確認します。
   - 予期しない結果が、長文コンテンツの弱い一致のみを共有しているかどうかを確認します。

1. **ノイズのあるフィールドがある場合とない場合のテスト：**

   - カテゴリパスおよび詳細な説明フィールドから検索可能なステータスを一時的に削除します。
   - 問題のあるクエリを再実行して、関連度が向上するかどうかを確認します。
   - 結果が改善された場合は、設定を永続的に調整します。

1. **例外に対してマーチャンダイジングルールを使用：**

   - 特別な処理が必要な既知のクエリについては、ターゲット設定された検索ルールを作成してください。
   - より多くの属性を検索可能にして、エッジケースを解決しようとしないでください。
   - ブランド名の検索やよくあるスペルミスには、リダイレクトを使用します。

1. **監視と繰り返し：**

   - [ パフォーマンスワークスペース ](performance.md) を使用して、ゼロ結果レートとクリックスルー率を追跡します。
   - 上位の検索クエリを毎週確認し、新しいパターンを特定します。
   - 前提データではなく、データに基づいて検索可能な属性と重み付けを調整します。
