...
- 次のフィードのインデックスが`Update by Schedule`に設定されていることを確認します。カタログ属性、製品、製品の上書き、製品バリアント。 管理者またはCLI （`bin/magento indexer:show-mode | grep -i feed`）を使用して、[Index Management](https://experienceleague.adobe.com/ja/docs/commerce-admin/systems/tools/index-management)のインデクサーを確認できます。
