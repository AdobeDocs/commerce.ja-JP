...
>[!NOTE]
>
>支払いレポートには、取り込まれた注文のみが表示されます（支払いアクションは[`Authorize and Capture`](https://experienceleague.adobe.com/docs/commerce-merchant-services/payment-services/get-started/production.html?lang=ja#set-payment-services-as-payment-method)に設定されています）。または[は`Invoiced`](https://experienceleague.adobe.com/ja/docs/commerce-admin/stores-sales/order-management/invoices#create-an-invoice)としてマークされています。
