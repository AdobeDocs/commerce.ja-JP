...
生産では、PayPal は購入者の位置情報を使用して、チェックアウトフローとエクスプレスフローで利用可能な支払い方法を決定します。 サンドボックスモードは位置情報をサポートしていないため、**買い手の国** 設定を使用して、買い手の場所をシミュレートし、レンダリングする支払い方法を制御します。

この設定は、VPN を必要とせずに、Venmo （米国のみ）、Pay Later （米国および英国）、[Local Payment Methods](payments-options.md#local-payment-methods) （ヨーロッパ）などの地域固有の支払い方法をテストする場合に役立ちます。
...
>[!NOTE]
>
>**[!UICONTROL Buyer's country]** 設定は、メソッドが `Sandbox` に設定されている場合にのみ表示されます。 これは、実稼動環境には影響しません。
