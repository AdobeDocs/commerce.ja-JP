...
このリリースノートは、[!DNL Payment Services] のすべてのリリースについて説明しており、以下が含まれています。
...
![ 新しい問題 ](../assets/new.svg)<!-- Issue PAY-6374 --> 新しく、**ダッシュボードの** 設定 [!DNL Payment Services] ボタンが支払い方法の管理設定ページにリダイレクトされるようになり、支払い設定を管理するワークフローが効率化されました。

_2026 年 1 月 19 日_

![ 新しい問題 ](../assets/new.svg)<!-- Issue PAY-6325 --> マーチャントは、[ トランザクションレポート ](reporting.md#transactions-report-view) の列として PayPal の販売者 ID を表示して、特定の顧客が行ったトランザクションを識別できるようになりました。
...
![ 新しい問題 ](../assets/new.svg)<!-- Issue PAY-5658 --> マーチャントは、[ トランザクションレポート ](reporting.md#transactions-report-view) の支払詳細でトランザクションをフィルタリングして、より詳細で正確な支払方法のデータを取得できるようになりました。

_2024 年 7 月 15 日_

![ 新しい問題 ](../assets/new.svg)<!-- Issue PAY-5571 --> マーチャントは、[ トランザクションレポート ](reporting.md#transactions-report-view) のCommerce顧客メールでトランザクションをフィルタリングできるようになりました。 特定の電子メールのトランザクションをフィルタリングする顧客の電子メールを入力します。

_2024 年 7 月 9 日_

![ 新しい問題 ](../assets/new.svg)<!-- Issue PAY-5488 --> マーチャントは、[ トランザクションレポート ](reporting.md#transactions-report-view) の列としてCommerce顧客 ID を表示することで、特定の顧客が実行した取引を特定できるようになりました。 さらに、マーチャントは、このCommerce顧客 ID でトランザクションレポートをフィルタリングして、関連する注文を確認できます。
...
![ 新規問題 ](../assets/fix.svg)<!-- Issue PAY-4888 --> マーチャントは、[ 取引レポート ](reporting.md#transactions-report-view) のカード番号の最後の 4 桁でクレジットおよびデビットカード取引をフィルタリングできるようになりました。
...
![ 新規 ](../assets/new.svg)<!-- Issue PAY-4288 --> 現在、マーチャントは、[_設定のみ_ PayPal 支払いボタン ](payments-options.md#use-only-paypal-payment-buttons) を使用できます _PayPal クレジットカード支払いオプションは使用できません_。 これにより、マーチャントは、Venmo や PayPal の支払いボタンなど様々な支払いオプションを提供したり、PayPal のクレジットカード支払いオプションの代わりに既存のクレジットカード会社を使用したりできます。

![ 新規 ](../assets/new.svg)<!-- Issue PAY-4050 --> 支払いサービスのホームに表示される、注文の支払いステータスレポート用の [ データビジュアライゼーションビュー ](/help/payment-services/payouts.md#payouts-data-visualization-view) を追加しました。
...
![ 新規 ](../assets/new.svg)<!-- Issue PAY-3705 --> `increment_id` は、外部 ERP システムでの支払い調整に使用できるようになりました。 `custom_id` _および_ `invoice_id` に伝播され、支払い用の PayPal Webhook およびマーチャントアクティビティ詳細の両方に表示されます。
...
![ 新規 ](../assets/new.svg)<!-- Issue PAY-3420 --> Apple Pay が PayPal スマートボタンとして使用できるようになりました。 この [ 支払いオプション ](payments-options.md#apple-pay-button) により、お客様はiOSまたはmacOS デバイスで Touch ID 機能を使用して、Apple Pay を選択できます。 Apple Pay は、デバイスに保存されているクレジットカードとデビットカードの支払資格情報を使用して支払いを処理します。

_2021 年 6 月 28 日_

![ 新規 ](../assets/new.svg)<!-- Issue PAY-1720 --> 店舗注文の争議が [ 注文支払ステータス レポート ](/help/payment-services/order-payment-status.md#view-disputes) で利用できるようになりました。 [!DNL Payment Services] から PayPal 解決センターに直接移動することで、紛争に対処できます。
...
![ 新規 ](../assets/new.svg)<!-- Issue PAY-2761 --> 列設定コントロールを使用して、[ 注文支払いステータスレポート ](/help/payment-services/order-payment-status.md#show-and-hide-columns) および [ 支払いレポート ](/help/payment-services/payouts.md#show-and-hide-columns) に表示されるデータをカスタマイズできるようになりました。
...
![ 新規 ](../assets/new.svg)<!-- PAY-6400 --> PayPal への行項目と金額の分類の送信を有効または無効にする **[!UICONTROL Line Items Enabled]** 設定を追加しました。 この設定は、サードパーティの拡張機能によって、[!DNL Payment Services] でサポートされていないカスタム料金が追加された場合のチェックアウトの問題を解決するのに役立ちます。 詳しくは、[ 行項目 ](line-items.md) を参照してください。

![ 新規 ](../assets/new.svg)<!-- PAY-6457 --> チェックアウトの開始時に [Express 支払いボタン ](payments-options.md#express-checkout-buttons) を追加して、チェックアウトの高速化を促進しました。

![ 新規 ](../assets/new.svg)<!-- PAY-6458 --> 地域固有およびローカルの支払いオプションに対して [ ローカル支払い方法（LPM） ](payments-options.md#local-payment-methods) のサポートを追加しました。 マーチャントは、[Commerce](configure-admin.md#local-payment-methods) 内で直接利用可能な LPM を有効または無効にできます。

![ 新規 ](../assets/new.svg)<!-- PAY-6377 --> [ 購入者の国 ](sandbox.md#buyers-country) サンドボックス設定を追加して、購入者の場所をシミュレートし、レンダリングされる支払い方法を制御します。

![ 問題を修正 ](../assets/fix.svg)<!-- PAY-6458 --> 購入者のメールアドレスデータを渡すことにより、PayPal の不正フィルターとの互換性が向上し、不正保護機能が強化されました。

![ 問題を修正 ](../assets/fix.svg)<!-- PAY-6413 --> 配送先住所が不明なために仮想商品の PayPal チェックアウトを完了できなかった問題を修正しました。

![ 修正された問題 ](../assets/fix.svg)<!-- PAY-6450 --> 製品の詳細ページ、ミニカート、買い物かごおよびチェックアウト上部で PayPal ボタンのチェックアウトエラーが正しく処理されない問題を修正しました。

![ 修正された問題 ](../assets/fix.svg)<!-- PAY-6453 -->GraphQLのチェックアウト時にサーバーサイド出荷コールバックのフローを無効にして、チェックアウト時の競合を防ぎます。

![ 修正された問題 ](../assets/fix.svg)<!-- PAY-6456 --> [Apple Pay](configure-admin.md#apple-pay) 設定の説明を更新し、Appleが未確認のドメインの支払いを拒否することを明確にしました。 マーチャントは、Apple Pay ボタンが使用されているドメインを検証する必要があります。 国の制限が適用されます。

## v2.13.3

_2026 年 1 月 14 日_

![ 問題を修正 ](../assets/fix.svg)<!-- PAY-6399 --> 管理者のチェックアウト中に間違ったボルト付きカードが使用された問題を修正しました。 これで、選択したカードが注文時に正しく使用されます。

## v2.13.2

_2026 年 1 月 5 日_

![ 修正された問題 ](../assets/fix.svg)<!-- PAY-6390 --> 改善された JS 縮小を実装することによる一般的な改善と最適化。

## v2.13.1

_2025 年 12 月 18 日_

![ 問題を修正 ](../assets/fix.svg)<!-- PAY-6355 --> マーチャントがチェックアウト中に配送方法を選択できなかった問題を修正しました。

![ 問題を修正 ](../assets/fix.svg)<!-- PAY-6347 --> 不要な API 呼び出しを削除することで、チェックアウトの復元性を向上しました。

![ 問題を修正 ](../assets/fix.svg)<!-- PAY-6368 --> プライマリドメインが利用できない場合に支払いSDKを読み込むためのフォールバックメカニズムを実装し、チェックアウト中にクレジットカードフィールドにアクセスできるようにします。
...
![ 新規 ](../assets/new.svg)<!-- PAY-xxxx --> [!DNL Payment Services] では、PayPal の **OTC （ワンタイムチェックアウト）** モーダルをサポートするようになりました。このモーダルでは、組み込みの連絡先と配信情報を利用できます。

![ 新機能 ](../assets/new.svg)<!-- PAY-xxxx --> **PayPal アプリスイッチ認証** を通じてモバイルエクスペリエンスが強化され、API 統合が改善されてユーザージャーニーがスムーズになりました。 この機能は、米国 [!DNL Payment Services] ベースのお客様のみが利用できます。

![ 新規 ](../assets/new.svg)<!-- PAY-xxxx --> **3D セキュア （3DS）認証** Fastlane のサポートが追加され、**Strong Customer Authentication （SCA）** の要件を満たすことができるようになりました。 これにより、英国および EU [!DNL Payment Services] のマーチャントは **3DS 認証** を使用してトランザクションを処理できるようになり、不正防止が強化され、地域の規制へのコンプライアンスが確保されます。

![ 新規 ](../assets/new.svg)<!-- PAY-xxxx -->Fastlane チェックアウトコンポーネントで、[!DNL Payment Services] マーチャントが **ライトテーマとダークテーマ** の中から選択できるようになり、チェックアウトページをサイトデザインと一致させることができます。 カスタムスタイルがアクセシビリティ標準を満たさない場合は、システムは自動的にデフォルト設定に戻ります。

![ 修正された問題 ](../assets/fix.svg)<!-- PAY-xxxx --> 3DS チャレンジでの管理者チェックアウト **中のローダーの問題を修正** ました。

![ 修正された問題 ](../assets/fix.svg)<!-- PAY-xxxx -->API を使用して **支払い設定を保存する際の検証** を改善しました。
...
![ 問題を修正 ](../assets/fix.svg)<!-- PAY-6164 --> 現在、[!DNL Payment Services] は、[PayPal サーバーサイド配送コールバック（SSSC） ](https://developer.paypal.com/docs/multiparty/checkout/standard/customize/shipping-module/) で使用可能な配送方法のベース通貨を使用しています。
...
![ 新規 ](../assets/new.svg)<!-- PAY-6022 --> [Fastlane](https://experienceleague.adobe.com/en/docs/commerce/payment-services/payments-checkout/payments-options) は、ゲストのチェックアウト時に購入を迅速化します。
...
![ 新規 ](../assets/new.svg)<!-- PAY-6227 -->PayPal でチェックアウトする場合、[!DNL Payment Services] は注文確認のポップアップをスキップして、購入プロセスを迅速に行います。 これは、PayPal レビューページ内で直接送料と合計を計算する [ サーバーサイドの送料コールバック ](payments-options.md#server-side-shipping-callback) によって有効にされています。

![ 新規 ](../assets/new.svg)<!-- PAY-6234 --> [ 後で支払う ](https://experienceleague.adobe.com/en/docs/commerce/payment-services/payments-checkout/payments-options) 支払いオプションに新機能を追加しました。 BNPL メッセージ・コンフィギュレータにより、顧客のチェックアウト・ページに後で支払う BNPL メッセージをより柔軟に表示できるようになりました。
...
![ 新機能 ](../assets/new.svg)<!-- PAY-5938 --> 現在は、マーチャント [!DNL Payment Services] 支払い設定を管理して、ビジネスの柔軟性を最大限に高めることができます。 このバージョンにより、マーチャントがサポートする地域とブランドに [ 複数の PayPal アカウント ](configure-admin.md#use-multiple-paypal-accounts) を添付する機能が向上します。 アドビのセールスチームは、web サイトとストアの表示範囲を設定するためのオンボーディングリンクを提供できます。
...
![ 新規 ](../assets/fix.svg)<!-- PAY-5789 --> [!DNL Payment Services] は、Google Pay[ での ](security.md#3ds)3D セキュア認証をサポートするようになり、支払い取引中のマーチャントおよびお客様のセキュリティを強化します。

![ 修正 ](../assets/fix.svg)<!-- PAY-5703 --> [!DNL Payment Services] では、[ 顧客が自分の **マイアカウント**](vaulting.md#vaulting-without-purchase) にカードを直接保存する機能が追加され、利便性が向上し、今後のチェックアウトが簡素化されます。`Vault without purchase functionality might not be 100% compatible with Adobe Commerce 2.4.4 due to a known issue with` [`GraphQL authorization mechanisms`](https://developer.adobe.com/commerce/webapi/graphql/usage/authorization-tokens/)。

![ 修正 ](../assets/fix.svg)<!-- PAY-5762 --> 注文が製品詳細ページ（PDP）から開始された際に、注文レビューページでクーポンコードが適用されない問題を修正しました。

![ 修正 ](../assets/fix.svg)<!-- PAY-5792 --> [!DNL Payment Services] では、[ チェックアウトページのヴォールトカード ](vaulting.md) の説明と請求先住所が表示されるようになり、顧客は保存済みの支払い方法をより明確に把握できるようになりました。
...
![ 新規 ](../assets/new.svg)<!-- PAY-5380 --> [!DNL Payment Services] では、マーチャント向けの [ 管理での設定 ](configure-admin.md#general-configuration) エクスペリエンスが改善され、より簡単で直感的なオンボーディングプロセスが実現しました。 この機能を使用すると、マーチャントは [!DNL Payment Services] ID をリセットできます。
...
![ 修正 ](../assets/fix.svg)<!-- PAY-5492 --> マーチャントがカスタマイズされた利用条件をチェックアウトページに追加した際の問題を修正しました。 [ エクスプレスチェックアウト ](payments-options.md#standard-vs-advanced-payments-experience) 中、買い物客はこれらの利用条件に同意して、問題なくチェックアウトを完了できるようになりました。
...
![ 新規 ](../assets/new.svg)<!-- PAY-4877 --> 現在、[!DNL Payment Services] は [L2/L3 の価格設定機能をサポートしています ](/help/payment-services/levels-card-payment-transactions.md#level-2-and-level-3)。 この機能は、IC++の価格が有効になっている [!DNL Payment Services] のお客様のみが利用できます。 [!DNL Payment Services] に L2/L3 処理データを使用する場合は、[!DNL Payment Services] のアカウントマネージャーにお問い合わせください。
...
![ 新機能 ](../assets/new.svg)<!-- PAY-4868 --> マーチャントは、管理者を通じて [ の他の支払いボタンと同様に、](/help/payment-services/payments-options.md) 購入エクスペリエンス全体を通してGoogle Pay を設定 [!DNL Payment Services] できます。
...
![ 修正 ](../assets/fix.svg)<!-- PAY-5291 --> 一部の取引について、支払い確認の受信が遅延する場合があります。 その場合、マーチャントは注文の更新された支払いステータスを取得できるようになりました。 [Payment Services は、保留中の取引を検出し ](/help/payment-services/order-payment-status.md#payment-status-updates) これらの取引をプロアクティブに監視し、保留中のステータスが取得されたときに更新することで、支払い取引の保留中のステータスを順番に検出します。
...
![ 新規 ](../assets/new.svg)<!-- PAY-4638 --> 自動の不正保護サービスを提供する [Signifyd との統合 ](fraud-protection.md) を追加しました。

![ 新規 ](../assets/new.svg)<!-- PAY-3981 --> [Apple Pay を別の支払いオプションに昇格 ](payments-options.md#apple-pay-button) し、PayPal 支払いボタンの外側で、買い物客の支払いオプションの可視性を高め、マーチャントがApple Pay の配置とスタイル設定を制御できるようにします。
...
![ 新規 ](../assets/new.svg)<!-- PAY-4002 --> マーチャントが特定の支払いオプションに優先順位を付ける [ 支払いオプションの順序を並べ替える ](configure-admin.md#paypal-payment-buttons) ことができる機能を追加しました。 この機能により、チェックアウトの会話率を高くすることをお勧めします。

![ 新規 ](../assets/new.svg)<!-- PAY-4035 --> マーチャントは、[Admin ホームページから使用できる新しい ](reporting.md#transactions-report-view) トランザクションレポート [!DNL Payment Services] を使用して、ストアの正常性を効率的に監視し、トランザクションの問題を特定できるようになりました。 このレポートには、トランザクションの承認レートと負のトランザクションのトレンドに関するデータも表示されます。
...
![ 新規 ](../assets/new.svg)<!-- Issue xxx -->[ オーストラリア、フランス、英国の国および関連通貨での提供 ](introduction.md#availability) を追加しました。

![ 新規 ](../assets/new.svg)<!-- Issue PAY-4296 --> 管理者ロール用に拡張されたリソース [ が追加されました ](configure-admin.md#configure-roles) 管理者ユーザーが顧客の注文を作成および管理でき、セールスメニューで表示できる [!DNL Payment Services] ようにします。

![ 新規 ](../assets/new.svg)<!-- Issue PAY-4236 -->[ チェックアウト中にエラーが発生する注文の自動無効化 ](checkout.md#order-auto-voided-if-error) を追加しました。

![ 新規 ](../assets/new.svg)<!-- Issue PAY-4183 --> チェックアウトページに [ クレジット/デビットカード支払いオプションボタンを表示 ](payments-options.md#paypal-debit-or-credit-card-button) する機能が作成されました。
...
![ 修正 ](../assets/fix.svg)<!-- Issue PAY-4205 --> [PayPal オプション ](payments-options.md#paypal-payment-buttons) を使用してチェックアウト時に、指定された配送先住所の州がデフォルトの税金設定の州に置き換えられる問題を修正しました。 これで、顧客は注文をマーチャントの税金設定でデフォルトとして設定された以外の州に出荷できます。

![ 修正 ](../assets/fix.svg)<!-- Issue PAY-4202 --> 顧客がカードボルトを使用して購入を完了したり、（[ の支払いアクションを使用して `Authorize and Capture` 店舗のボルトを使用した支払い方法を削除したりできない問題を修正しました ](production.md#set-payment-services-as-payment-method)。 以前は、お客様がボールトに登録されているクレジットカードを使用または変更しようとすると、「Provider Vault ID が見つかりません」というエラーが表示されていました。
...
![ 新規 ](../assets/new.svg)<!-- Issue PAY-3880 --> 買い物客は、チェックアウト時にクレジットカード情報を [vault （保存） ](vaulting.md) し、後で同じ販売者アカウント内の同じ店舗または別の店舗で使用できるようになりました。

![ 新機能 ](../assets/new.svg)<!-- Issue PAY-3950 --> マーチャントは、店舗に対して [ インスタント購入Commerce機能 ](https://experienceleague.adobe.com/docs/commerce-admin/stores-sales/point-of-purchase/checkout-instant-purchase.html) を有効にできるようになりました。これにより、買い物客は（[ ボールトに登録されたクレジットカード情報 ](vaulting.md) を使用）でチェックアウトを迅速化できます。
...
![ 新規 ](../assets/new.svg)<!-- Issue PAY-784 -->[!DNL Payment Services] に、マーチャントアカウントを設定して [ 複数の PayPal ビジネスアカウントを使用 ](configure-admin.md#use-multiple-paypal-accounts) する機能が含まれるようになりました。 これにより、マーチャントは、異なる通貨を使用して複数の国で店舗を運営したり、ビジネスの一部にAdobe Commerceを使用したりできます。

![ 新規 ](../assets/new.svg)<!-- Issue PAY-3231 --> マーチャントは、[ 顧客トランザクションの銀行取引明細書に表示される、ブランド、ストアまたは製品ラインを記述する [!UICONTROL Soft Descriptor]](configure-admin.md) ールを web サイトまたは個々のストアビューに追加できます。

![ 新規 ](../assets/new.svg)<!-- Issue PAY-3707 --> [ クレジットカードのフィールドと PayPal の支払いボタンを有効または無効にする ](configure-admin.md#paypal-payment-buttons) チェックアウト [!DNL Payment Services] 設定。
...
![ 新規 ](../assets/new.svg)<!-- Issue PAY-2682 --> カナダのマーチャントが [!DNL Payment Services] および [!DNL Adobe Commerce] の [!DNL Magento Open Source] 拡張機能を使用できるようになりました。 マーチャントは、支払設定を [ フランス語 ](introduction.md?lang=fr#accepted-credit-cards-and-currencies) または [ 英語 ](introduction.md#accepted-credit-cards-and-currencies) で表示できます。
...
![ 既知の問題 ](../assets/bug.svg)<!-- Issue PAY-2473 --> 拡張機能のインストール中に [Composer キーが正しくない ](https://experienceleague.adobe.com/docs/commerce-knowledge-base/kb/troubleshooting/payments/payservices-install.html) を使用すると、ユーザーは正しい [ で ](https://experienceleague.adobe.com/en/docs/commerce-operations/installation-guide/prerequisites/authentication-keys) 認証 `MAGEID` できません。
