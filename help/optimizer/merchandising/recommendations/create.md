...
1. _[!DNL Adobe Commerce Optimizer]_&#x200B;サイドバーで、_ マーチャンダイジング _/**レコメンデーション**&#x200B;に移動して_ レコメンデーション _ワークスペースを表示します。

1. 「**カタログ表示**」フィールドで、レコメンデーションを使用可能にするカタログ表示を選択します。 詳しくは、[&#x200B; レコメンデーションへのカタログビューの使用 &#x200B;](../../manage-results/recommendation-performance.md#select-catalog-view) を参照してください。
...
1. **レコメンデーションを作成** をクリックします。
   ...
   作成するレコメンデーションは、以前に選択したカタログビューで使用できます。
...
1. _推奨製品プレビュー_ パネルを使用すると、フィルターがレコメンデーションユニットに表示される製品に与える影響をより深く理解できます。 詳しくは、[&#x200B; レコメンデーションのプレビュー &#x200B;](#preview-recommendations) を参照してください。
...
1. 完了したら、次のいずれかをクリックします。
   ...
   レコメンデーションが Recommendations ワークスペースに表示されます。 ストアフロントでレコメンデーションを使用するには、[&#x200B; レコメンデーション ID](#get-recommendation-id) を見つける必要があります。

>[!NOTE]
>
>最大 50 個のアクティブなレコメンデーションユニットを作成できます。 詳細については、[&#x200B; 境界と境界 &#x200B;](../../boundaries-limits.md) を参照してください。
...
![Recommendations のプレビュー &#x200B;](../../assets/rec-preview.png)
...
マルチストアフロント、多言語、マルチブランドの設定の場合、各レコメンデーションをすべてのカタログ表示（グローバル）に適用するか、単一の [&#x200B; カタログ表示 &#x200B;](../../setup/catalog-view.md) に適用するかを設定できます。 レコメンデーションを使用する際の [&#x200B; カタログ表示の設定 &#x200B;](../../manage-results/recommendation-performance.md#select-catalog-view) 方法について詳しくは、こちらを参照してください。

## レコメンデーション ID を取得

レコメンデーションを作成したら、その ID を取得して、ストアフロントにレコメンデーションユニットを実装する必要があります。

1. **レコメンデーション** ページで、レコメンデーションを選択します。

1. レコメンデーション名の横にある情報アイコン（![&#x200B; 情報アイコン &#x200B;](../../assets/info-icon.png)）をクリックします。

   **レコメンデーションユニットの詳細** ページが表示されます。

   ![&#x200B; レコメンデーション ID の取得 &#x200B;](../../assets/get-rec-id.png)
...
1. _[!DNL Adobe Commerce Optimizer]_&#x200B;サイドバーで、_ マーチャンダイジング _/**レコメンデーション**&#x200B;に移動します。
...
1. メニューで、レコメンデーションを **ディアクティベート**、**削除** または **編集** できます。 **編集** を選択した場合は、必要に応じて次の設定を調整できます。
   ...
   レコメンデーションタイプやカタログビューは変更できません。 カタログ表示は、レコメンデーションを作成する際に設定されます。 詳しくは、[&#x200B; カタログビューを選択 &#x200B;](../../manage-results/recommendation-performance.md#select-catalog-view) を参照してください。

1. 完了したら、「**変更を保存**」をクリックします。

## 準備状況インジケーター

準備状況インジケーターは、使用可能なカタログと行動データに基づいて、どの推奨タイプが最もパフォーマンスが高いかを示します。 また、[&#x200B; イベント収集 &#x200B;](../../setup/events/overview.md) で発生する可能性のある問題を特定したり、結果を生成するのに十分なトラフィックがレコメンデーションタイプに届いていないかどうかを判断したりするのに役立ちます。
...
これらの変数の結果、準備指標の割合が変動する可能性があります。 この変動は、レコメンデーションタイプが「デプロイの準備完了」であることの内外に現れる理由を説明します。
...
- 少なくとも 50% は、そのレコメンデーションタイプに対して提案されたレコメンデーションの関連性が低いことを意味します。
...
マーチャントのカタログはほとんど変更されないので、カタログデータに依存するレコメンデーションタイプの準備状況インジケーターはあまり変更されません。 ただし、買い物客の行動データに基づくレコメンデーションタイプの準備状況インジケーターは、多くの場合、毎日の買い物客のアクティビティに応じて変化します。
...
- **動的ベース** – 動的ベースの指標の割合が低い場合は、次の原因が考えられます。
   ...
   - ストアへのトラフィックが少ないため、受信する行動イベントの量は少なくなります。
