...
1. **[ストアフロント設定のカスタマイズ](#configure-your-storefront)** - リポジトリの `config.json` ファイルを更新して、[!DNL Adobe Commerce Optimizer] インスタンスに接続します。
...
* **Commerce設定**: [Commerce ストアフロント設定 &#x200B;](https://experienceleague.adobe.com/developer/commerce/storefront/setup/configuration/commerce-configuration/?lang=ja){target="_blank"} インスタンス固有の設定など
...
1. プレースホルダーの値をインスタンスの値に置き換えます。 [&#x200B; 前提条件 &#x200B;](#prerequisites) を参照してください。
   ...
   >[!NOTE]
   >
   >価格台帳 ID を検索するには、[&#x200B; カタログ表示構成詳細 &#x200B;](./setup/catalog-view.md) をチェックして、割り当てられ [!DNL Adobe Commerce Optimizer] 価格台帳を確認します。 価格台帳が割り当てられていない場合は、構成ファイルからこのヘッダーを削除できます。 価格台帳がカタログ ビューに割り当てられたら、再度追加します。
...
| **製品データが表示されません** | 製品ページにプレースホルダーまたはエラーが表示される | <ul><li>`config.json` で設定値を確認します。</li><li>[!DNL Adobe Commerce Optimizer] インスタンスで、データ同期ページをチェックして、サンプル製品が読み込まれていることを確認します。 使用できる製品がない場合は、サンプルデータを再読み込みするか、[Data Ingestion API](https://developer.adobe.com/commerce/services/optimizer/data-ingestion/using-the-api/#make-your-first-request) を使用して製品を追加します。 設定変更が反映されるまで数分待ちます。</li><li>[Commerce ストアフロント設定 &#x200B;](https://developer.adobe.com/commerce/services/optimizer/merchandising-services/use-cases/#return-product-details) で設定されたのと同じヘッダーを使用して、マーチャンダイジングサービス [products クエリ &#x200B;](https://experienceleague.adobe.com/developer/commerce/storefront/setup/configuration/commerce-configuration/?lang=ja){target="_blank"} を使用して商品の詳細を取得してみてください。 データを取得できる場合は、カタログ表示の設定またはインデックスエラーの問題がある可能性があります。</li></ul> |
| **検索で結果が返されない** | 空の検索結果ページ | <ul><li>[Commerce ストアフロントの設定 &#x200B;](https://developer.adobe.com/commerce/services/optimizer/merchandising-services/use-cases/#product-search) で設定されたのと同じヘッダーを使用して、マーチャンダイジングサービス [productSearch クエリ &#x200B;](https://experienceleague.adobe.com/developer/commerce/storefront/setup/configuration/commerce-configuration/?lang=ja){target="_blank"} を使用して商品の検索結果を取得できることを確認します。 データを取得できる場合は、カタログ表示の設定またはインデックスエラーの問題がある可能性があります。</li><li>[Commerce ストアフロント設定のカタログビュー ID が &#x200B;](https://experienceleague.adobe.com/developer/commerce/storefront/setup/configuration/commerce-configuration/?lang=ja){target="_blank"}[!DNL Adobe Commerce Optimizer] のカタログビュー ID と一致していることを確認します。</li><li>ま [!DNL Adobe Commerce Optimizer]、ストアフロントのヘッダー設定で使用したポリシー、ロケール、価格台帳の設定を確認します。</li><li>[&#x200B; 属性メタデータ設定 &#x200B;](https://developer.adobe.com/commerce/services/reference/rest/#operation/createProductMetadata) が検索に対して正しく設定されていることを確認します。</li></ul> |
...
* [[!DNL Adobe Commerce Optimizer]  開発者ガイド &#x200B;](https://developer.adobe.com/commerce/services/optimizer/) を確認してください。
