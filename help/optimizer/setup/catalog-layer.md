...
* **商品ビジュアル** - AEM Assetsの商品画像を、基本カタログの上のレイヤーとして適用します。

## AEMAssets層

[ 商品ビジュアル ](product-visuals.md) を有効にすると、AEM Assets統合によって、AEM Assets コンテンツ専用のカタログレイヤーが自動的に作成および管理されます。 デフォルトのレイヤー名は `AEM-Assets` ですが、[AEM Assets統合でのオンボーディング ](../../aem-assets-integration/get-started/configure-aco.md) 中にカスタム名を指定できます。

このレイヤーには、AEM Assetsから同期された商品画像が含まれます。 他のカタログレイヤーと同様に、[Product Layers API](https://developer.adobe.com/commerce/services/reference/rest/#tag/Product-Layers){target=_blank} を使用して入力されます。 Assets Integration Service は、AEMのアセットのメタデータと配信 URL を API 形式に変換し、AEM Assetsでアセットが承認されると、そのデータを自動的に送信します。

統合では、テナントごとに 1 つのソース（1 つのロケール + 1 つのレイヤー）をサポートしています。

>[!CAUTION]
>
> AEM - Assets レイヤーをカタログビューに割り当てます。 レイヤーが割り当てられていない場合、製品画像データが予期せず上書きされる可能性があります。

### AEM-Assets レイヤーの仕組み

1. **自動作成**：レイヤーは、AEM Assets統合が [!DNL Commerce Optimizer] インスタンス用に設定されたときに作成されます。

1. **画像の同期**:AEM Assetsでアセットが承認されると、Assets Integration Service がアセットデータを変換し、製品レイヤー API を使用して `AEM-Assets` レイヤーを更新します。

1. **レイヤーの割り当て**:AEM Assets画像を表示するカタログビューに `AEM-Assets` レイヤーを割り当てます。

### AEM-Assets レイヤーのカタログビューへの割り当て

ストアフロントにAEM Assets画像を表示するには：

1. _ストア設定_ に移動し、「**[!UICONTROL Catalog views]**」をクリックします。

1. 画層を適用するカタログ ビューを選択します。

1. 「カタログレイヤー」セクションで、**AEM-Assets** レイヤーを見つけます。

1. 画層をアクティブにして、このカタログ ビューで有効にします。

1. 「**[!UICONTROL Save]**」をクリックして変更を適用します。

割り当てられると、ストアフロント API （カタログサービス、ライブ検索、商品レコメンデーション、ストアフロント GraphQL API）は、商品の基本カタログ画像とAEM Assets画像の両方を返します。

商品ビジュアルの設定について詳しくは、[AEM Assetsでの商品ビジュアル ](product-visuals.md) を参照してください。
...
API の仕様とペイロードの例について詳しくは、開発者向けドキュメントの [ 製品レイヤー ](https://developer.adobe.com/commerce/services/reference/rest/#tag/Product-Layers){target=_blank} を参照してください。
...
データ取り込み API を使用して [ カタログレイヤーを削除 ](https://developer.adobe.com/commerce/services/reference/rest/#operation/deleteProductLayers){target=_blank} します。
...
* [ 商品ビジュアル ](product-visuals.md) – 商品画像にAEM Assetsを使用する
