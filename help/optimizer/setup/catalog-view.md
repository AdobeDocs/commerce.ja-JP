...
カタログ表示は、[!DNL Adobe Commerce Optimizer] マーチャンダイジングサービスの基盤であり、ビジネス構造、ポリシー、価格によって製品カタログを整理できます。 この柔軟なデータモデルは、運用効率を維持しながら、マルチブランド、マルチビジネスユニット、多言語シナリオをサポートします。
...
次に、カタログビューとカタログソースの違いについて説明します。

- **カタログビュー** – 特定のビジネスニーズに対応するためのカタログの設定済みビューです。 カタログ表示を作成する場合は、使用するカタログソース（またはロケール）を選択し、表示する製品をフィルタリングする [&#x200B; ポリシー &#x200B;](policies.md) を追加してから、価格を制御する [&#x200B; 価格台帳 &#x200B;](pricebooks.md) をリンクします。 単一のカタログソースで多くのカタログビューを強化できます（例えば、1 つの `en-US` ソースに異なるブランドや地域の個別のカタログビューを含めることができます）。 カタログビューは、そのデータをストアフロント *チャネル）またはオーディエンスに公開する* 方法）と考えることができます。

- **カタログソース** – 製品情報を提供する基になるデータコンテキスト。 カタログソースは、通常、ロケール（`en-US`、`fr-CA` など）または外部システム（PIM や ERP など）です。 [!DNL Adobe Commerce Optimizer] は、1 つ以上のカタログソースから統合された基本カタログに製品データを取り込みます。 カタログソースは、生のカタログデータの元となる *場所* と考えてください。

要約すると、**カタログソース** は取り込むデータコンテキストであり、**カタログビュー** は、買い物客やチャネルに表示する、フィルタリング、価格設定、整理されたコンテキストです。
...
### レコメンデーションと製品検出ルールのカタログビューの指定

>[!IMPORTANT]
>
>この機能は現在ベータ版です。

カタログ表示を指定できるのは、[&#x200B; レコメンデーションユニットの作成 &#x200B;](../merchandising/recommendations/create.md) または [&#x200B; マーチャンダイジングルール &#x200B;](../merchandising/rules/add.md) の場合です。
...
| コンポーネント | 目的 | 例 |
|---|---|---|
| **カタログ表示** | ビジネスユニットまたは配布チャネル | ディーラーネットワーク、地域店舗 |
