...
- **ロケールなどの属性に基づいて、どのカタログソースが使用されるか**
...
次に、カタログビューとカタログソースの違いを説明します。

- **カタログ ビュー** – 特定のビジネス ニーズに合わせてカタログの設定済みビュー。 カタログビューを作成する際に、使用するカタログソース（またはロケール）を選択し、[&#x200B; ポリシー](policies.md)を追加して、表示される製品をフィルタリングし、[価格表](pricebooks.md)をリンクして価格設定を制御します。 単一のカタログソースを使用すると、多数のカタログビューを利用できます（例えば、ブランドや地域ごとに別々のカタログビューを持つ1つの`en-US` ソース）。 カタログビューは、そのデータをストアフロント、チャネル、オーディエンスに公開する&#x200B;*方法*&#x200B;と考えてください。

- **カタログ ソース** – 製品情報を提供する基礎となるデータ コンテキスト。 カタログソースは、通常、ロケール（`en-US`、`fr-CA`など）またはPIMやERPなどの外部システムです。 [!DNL Adobe Commerce Optimizer]は、1つ以上のカタログ ソースから製品データを統合された基本カタログに取り込みます。 カタログソースは、生のカタログデータの出所である&#x200B;*どこ*&#x200B;と考えてください。

要約すると、**カタログソース**&#x200B;は取得するデータコンテキストです。一方、**カタログビュー**&#x200B;は、買い物客またはチャネルに提示する、フィルタリングされ、価格設定され、整理されたコンテキストです。
...
1. カタログビューの詳細を設定します。
   ...
   - **カタログ ソース** - カタログ ソース （ロケール）を選択します（例：`en-US`）。
