...
組織がプロビジョニングされたタイミングに基づいて、製品管理者ユーザーをAdobe Commerce Optimizerに追加する方法は 2 つあります。 アーリーアクセス組織では、製品管理者の役割を割り当てられた各ユーザーには、組織内のすべてのインスタンスを管理する権限があります。 2025 年 10 月 13 日（PT）以降にプロビジョニングされた一般提供（GA）組織では、特定のインスタンスの製品管理者としてユーザーを割り当てることができます。 製品管理者ユーザーがログインすると、管理権限を持っているインスタンスのみが表示されます。
