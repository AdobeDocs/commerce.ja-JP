...
* **Commerce Optimizer** – このアプローチを使用すると、Commerce Optimizerを移行段階として使用して、複雑なカスタマイズとデータを自分のペースで [!DNL Adobe Commerce as a Cloud Service] に移行することで、繰り返し移行できます。 Commerce Optimizerを使用すると、カタログビューとポリシーを活用したマーチャンダイジングサービス、Edge Deliveryを活用したCommerce Storefront などにアクセスで [!DNL Product Visuals powered by AEM Assets] ます。
...
* **Assets Management** – すべての Assets Management には移行が必要です。 既に [!DNL AEM Assets] を使用している場合は、移行する必要はありません。
