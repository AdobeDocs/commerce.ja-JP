...
- **2026 年第 1 四半期** （まだ利用できません） – 一括データ移行ツールの最初のリリースが完了したら、サポートチケットを送信してツールにアクセスできるようになります。
- **2026 年第 1 四半期** （未公開） – 一括データ移行ツールの公開リリース後、このページからアクセスできるようになります。
...
カタログサービスは、このデータをライブ検索および製品レコメンデーションと共有します。 このプロセスでは、手動の介入は必要ありません。 データは、取り込みが完了すると、すべてのサービスで利用できるようになります。

>[!IMPORTANT]
>
>設定は、自動的には読み込まれません。 データ移行プロセスを開始する前に、[!DNL Commerce Admin] ージの現在のカタログ設定をメモします。 次に、ターゲット [!DNL Adobe Commerce as a Cloud Service] 環境に同じ設定を実装します。
