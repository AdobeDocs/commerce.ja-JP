...
![新しい問題](../assets/new.svg)新しい`asset_matches`属性を持つ[ カスタム自動一致](https://experienceleague.adobe.com/en/docs/commerce/aem-assets-integration/synchronize/custom-match){target=_blank} エンドポイントを更新しました。
...
![新しい問題](../assets/new.svg)<!-- Issue ACAP-1078 -->新しい`asset_matches`属性を持つ[ カスタム自動一致](https://experienceleague.adobe.com/en/docs/commerce/aem-assets-integration/synchronize/custom-match){target=_blank} エンドポイントを更新しました。 この変更により、特定の`productSku`に関連付けられたすべてのアセットを返すために、独自のマッチングロジックを実装できます。
