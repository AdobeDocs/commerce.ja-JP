...
テナント登録用のサポートチケット ](#onboarding)を[発行する&#x200B;**前**&#x200B;のAEM Assets手順を完了します。 インストールパターンはAdobe Commerce as a Cloud Serviceと一致します。[Commerce メタデータをサポートするようにAEM Assets プロジェクトを設定する](configure-aem.md)を参照してください。
