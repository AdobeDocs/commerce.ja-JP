...
* [ ユーザー権限と IMS](permissions.md) - アセットセレクターおよび自動入力設定フィールド（プログラム ID、環境 ID、ドメインマッピング）に必要です。
...
1. [!BADGE PaaS のみ ]{type=Informative tooltip="クラウドプロジェクト上のAdobe Commerceにのみ適用されます（Adobeが管理する PaaS インフラストラクチャ）。"}**[!UICONTROL Asset Selector IMS Client ID]** を入力します。
   ...
   この ID は、アセットセレクターおよび「プログラム ID」フィールドと「環境 ID」フィールドの自動入力機能を有効にするために必要です。 この ID を取得するには、[ ユーザー権限と IMS](permissions.md) を参照してください。 アセットセレクターについて詳しくは、[ 手動によるアセットの選択 ](../synchronize/asset-selector-integration.md) を参照してください。

1. AEM Assets環境 **[!UICONTROL Program ID]** と **[!UICONTROL Environment ID]** をドロップダウンメニューから選択します。

   ドロップダウンは、ユーザーの IMS セッションに基づいて自動入力されます。 この機能を使用するには、権限 [ ユーザー権限と IMS](permissions.md#user-permissions-and-ims) を持っていることを確認します。
