...
**AEM アセットセレクター** は、AEM Assets プロジェクトの IMS クライアント ID がCommerce管理者で設定され、ユーザーに必要な [ 権限と IMS 認証 ](../get-started/permissions.md) がある場合に使用できます。 [AEM アセットセレクターの設定 ](#configure-the-aem-asset-selector-in-adobe-commerce) を参照してください。
...
* [!BADGE SaaS のみ ]{type=Positive url="https://experienceleague.adobe.com/en/docs/commerce/user-guides/product-solutions" tooltip="Adobe Commerce as a Cloud ServiceおよびAdobe Commerce Optimizer プロジェクトにのみ適用されます（Adobeで管理される SaaS インフラストラクチャ）。"}Edge Delivery Servicesを活用したCommerce ストアフロントでAssetsを直接割り当てて、コンテンツの視覚効果を高めます。
...
1. **[!UICONTROL IMS Client ID]** フィールドに入力します。 必要な権限とこの ID の取得方法については、[ ユーザー権限と IMS](../get-started/permissions.md) を参照してください。
