...
製品画像は、一致ルールを介してリンクされます。

* AEM Assets（メタデータ、SKU リンク、承認）で商品アセットを追加または更新する方法については、[&#x200B; デフォルトの自動マッチング &#x200B;](synchronize/default-match.md) を参照してください。
* カテゴリ画像またはページビルダーのコンテンツについては、[&#x200B; 手動のアセット選択 &#x200B;](synchronize/asset-selector-integration.md) を参照してください。
...
1. 「**画像とビデオ** セクションを開きます。
   ...
   商品アセットを設定（画像を SKU にリンク）するには、AEM Assets オーサーインスタンスを開き、メインビューから **Assets** をクリックします。 メタデータ設定手順については、[&#x200B; デフォルトの自動マッチング &#x200B;](synchronize/default-match.md) を参照してください。
...
* [&#x200B; カスタム自動照合 &#x200B;](synchronize/custom-match.md)。

### 同期 SLA

同期タイミングについて詳しくは、「[&#x200B; 同期SLA](get-started/setup-synchronization.md#synchronization-sla)」を参照してください。
...
[!BADGE PaaS のみ &#x200B;]{type=Informative tooltip="クラウドプロジェクト上のAdobe Commerceにのみ適用されます（Adobeが管理する PaaS インフラストラクチャ）。"} 画像を埋め込むために **AEM アセットセレクター** を使用するには、ユーザーが必要な [&#x200B; 権限および IMS 認証 &#x200B;](get-started/permissions.md) を持っていることを確認します。
...
[AEM アセットセレクターを設定し &#x200B;](synchronize/asset-selector-integration.md) ユーザーが必要な [&#x200B; 権限と IMS 認証 &#x200B;](get-started/permissions.md) を持っていることを確認したら、それを使用してカタログカテゴリコンテンツにアセットを追加できます。
...
Commerce側のワークフローでメタデータを使用してアセットを商品にリンクする方法については、[&#x200B; デフォルトの自動照合 &#x200B;](synchronize/default-match.md) を参照してください。
