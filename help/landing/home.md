...
- **ストアフロントサービス** – 製品の検出、レコメンデーション、支払いに関する AI を活用した機能
- **Integration Services** - Adobe Experience Platform、AEM Assets、その他のAdobe ソリューションへの接続
...
これらの機能に加えて、Commerce インスタンスから SaaS プラットフォームへのデータの流れをモニタリングできるツールもあります。 これらのデータツールは、データを自動的に同期し、パフォーマンスの最適化に役立ちます。
...
<tr style="border: 0;">
   ...
   <td valign="top">
      ...
      <p>
         <em> 管理 UI を使用して、App Builder アプリケーションをCommerce インスタンスに関連付け、設定および管理します。</em>
      </p>
   </td>
</tr>
</table>
...
Adobe Commerce サービスは、統合プラットフォームを基盤として構築されています。 複数のサービスを使用する場合、次の利点があります。

- **統合データパイプライン** – 製品、価格、在庫データがすべてのサービスで自動的に同期され、重複したデータ入力やカスタム統合が不要になります。
- **リアルタイムパーソナライゼーション** – 検索、ブラウジング、購入などによる顧客の行動により、インテリジェントなレコメンデーションが強化され、ストアフロント全体にわたるランキングが強化されます。
- **一貫したストアフロントの統合** - Edge Delivery Services、PWA Studio、ヘッドレス実装を活用したCommerce ストアフロントを使用しているかどうかにかかわらず、同じドロップダウンと API がすべてのプラットフォームでシームレスに機能します。
- **マルチストアおよび多言語サポート** – すべてのサービスは、追加のセットアップを行わなくても、ストアビュー、顧客セグメント、B2B カタログ設定を自動的に尊重します。
- **リッチな商品ビジュアル** - AEM Assetsと統合すると、最適化された商品画像が、検索結果、商品ページ、レコメンデーションをまたいで一貫して表示されます。
- **Connected customer data** - データ接続を使用すると、ショッピング行動をAdobe Experience Platform、Real-Time CDP、Journey Optimizerと共有し、クロスチャネルのパーソナライゼーションとキャンペーンの最適化を可能にできます。

例えば、買い物客がライブ検索を使用して製品を検索し、製品レコメンデーションを表示した後に買い物かごに追加し、支払いサービスで購入を完了すると、このアクティビティはすべて統合データパイプラインを介してシームレスにフローされます。 この行動データにより、今後の買い物客に対して、より優れた検索結果とより関連性の高いレコメンデーションが強化されます。

>[!NOTE]
>
>サービス間で行動データ共有を有効にするには、イベントデータ収集を設定する必要があります。 設定手順については、[Commerce サービスの設定 &#x200B;](saas.md#saas-configuration) を参照してください。

各サービスは個別に使用できますが、それらを組み合わせることで、よりインテリジェントでパーソナライズされたショッピングエクスペリエンスが生まれます。
