...
[!DNL Product Recommendations]のFastly Image Optimization サポートはオプションのモジュールであり、個別にインストールされます。 このモジュールは、[Fastly Image Optimization](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/cdn/fastly) パラメーターを[!DNL Product Recommendations]の画像URLに適用します。 インストールするには、次のコマンドを実行します。
